...
Estimate_DCC_t <- function(mY) {
  ...
  # Estimate a Multivariate Student's t on the standardized residuals
...
### Estimate a bivariate Student's t DCC in two step with QML for the first step ###
Fit_DCC_t = Estimate_DCC_t(mY)

### Compare the filtered correlation of the two models
df <- data.frame(
...
# The model selected by BIC is DCC_t (i.e. the Student's t DCC model)
